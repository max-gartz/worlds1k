...
The pixel decoder (phase 2) maps level-1 latents back to frames. The same pattern applies to audio: an audio decoder maps level-2 latents back to mel spectrograms, which an off-the-shelf vocoder (HiFi-GAN, Vocos) converts to waveform.
...
Dreamer: z_0 → z_1 → z_2 → ... (latent trajectory)
  ├─ PixelDecoder:  z_t^(1) → frames → video
